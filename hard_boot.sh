...
usage () {
    ...
    echo -e "\n\t$0 [-h <HMC>] [-u <HMC user name>] [-p <HMC password>] [-m <machine>] [-t <time> {h|H}] [-U <LPAR user name>] [-P <LPAR Password>]\n"
...
#                        HOURS=$((HOURS+1))
#                else
#                        echo -e "\nOption: -t, Time should be always greater than 2 hours.\n"
#                        usage
#                        exit 1
#                fi
#        else
#                echo -e "\nOption: -t, Invalid Argument :- ${TIME}\n"
#                usage
#                exit 1
#        fi
#fi
...
then
            ...
            then
                        ...
                        do
                            sshpass -p "$LPARPASS" ssh -k -oLogLevel=quiet -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null ${LPARUSER}@${E} "echo ssk" > /dev/null
                            if [[ $? -eq 0 ]]
                            then
...
#for H in $(echo $FINAL_LPARS | tr " " "\n")
#do
#    timeout 2m sshpass -p "$LPARPASS" ssh -k -oLogLevel=quiet -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null ${LPARUSER}@${H} "hcl -run -mdt mdt.bu;htxcmdline -bootme on mode:hardf period:3" > /dev/null
#       if [[ $? -eq 0 ]]
#    then
#            HB_LPARS=("${HB_LPARS[@]}" $H)
#                   MyCount=$((MyCount+1))
...
#if [[ "${#FINAL_LPARS[@]}" !=  "${#HB_LPARS[@]}" ]]
#then
#        ECHO "Unable to start the hard boot on couple of LPARS, shutting down those LPARS ... \c"
#           if [[ "${#FINAL_LPARS[@]}" -gt "${#HB_LPARS[@]}" ]]
#           then
#                for I in $(echo $FINAL_LPARS | tr " " "\n")
#                do
#                    if [[ ! "${HB_LPARS[@]}" =~ "$I" ]]
#                    then
#                                           shutdown_lpar ${MACHINE} ${I}
...
#ECHO "Hard Boot is going to run for ${BLUE}${HOURS}${EC} hours."
ECHO "Total number of LPARs that are part of Hard Boot ... ${BLUE}${MyCount}${EC} LPAR(s)."
HB_START_TIME=$(date)
ECHO "Start Time ... ${BLUE}${HB_START_TIME}${EC}"
#HB_END_TIME=$(date -d "+${HOURS} hours")
#if [[ $? -ne 0 ]]
#then
#        echo "Failure Analysis:- $HB_END_TIME"
#        exit 1
#fi
